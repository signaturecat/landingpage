...
Otwórz [app.signature.cat](https://app.signature.cat) i kliknij **Zaloguj się przez Google**, używając konta firmowego. Na tym etapie SignatureCat prosi wyłącznie o podstawowe zakresy logowania `openid email profile` - uprawnienia Workspace są nadawane osobno w kroku 4, i to wyłącznie Twojemu własnemu, odizolowanemu kontu usługi.

Prywatne konta Gmail są odrzucane: SignatureCat wymaga konta Google Workspace.

## Krok 2: Zaakceptuj warunki

Zaraz po pierwszym zalogowaniu trafiasz na ekran **Zaakceptuj warunki, aby zacząć**. Zaznacz "Zapoznałem(-am) się z Regulaminem i Polityką prywatności i akceptuję ich postanowienia." (oba dokumenty otwierają się w nowej karcie) i kliknij **Akceptuję i przechodzę dalej**.

Ten krok jest obowiązkowy i celowo jest pierwszy: dopóki warunki nie zostaną zaakceptowane, dla Twojej organizacji nie jest tworzone nic.

Zaakceptować je może wyłącznie administrator, który założył organizację. Każda inna osoba z Twojej domeny, która zaloguje się wcześniej, zobaczy kartę **Czekamy na Twojego administratora** i dostanie dostęp, gdy tylko administrator założyciel wykona ten krok.

## Krok 3: Skonfiguruj swój workspace

Gdy warunki są zaakceptowane, trafiasz na ekran **Skonfiguruj swój workspace**. Kliknięcie **Skonfiguruj workspace** tworzy dedykowane, odizolowane konto serwisowe Google Cloud dla Twojej organizacji - to tożsamość, która będzie zarządzać podpisami Gmail w Twoim imieniu. Zwykle zajmuje to kilka sekund (do 15).
...
## Krok 4: Autoryzuj Domain-Wide Delegation
