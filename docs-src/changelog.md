...
- **Find people by name, not just email.** Every user picker in the app - the editor's "render as" field, [Logs](/docs/logs), [User data](/docs/user-data) and a brand-new search when granting access in [User management](/docs/user-management) - now matches by first name, last name and full name: "Tomasz", "Piasecki" and "Tomasz Piasecki" all find the same person, and names with diacritics like "Łukasz" work as typed.
