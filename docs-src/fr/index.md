...
4. **Comblez les manques.** Là où l'annuaire est incomplet, un administrateur peut stocker sur l'onglet **Données** des valeurs par utilisateur qui l'emportent sur l'annuaire dans les signatures. Voir [Données utilisateur](/docs/user-data).
5. **Restez à jour.** Une synchronisation en arrière-plan réapplique les affectations une fois par jour, de sorte que les nouvelles recrues et les changements d'équipe reçoivent la bonne signature sans que personne ne lève le petit doigt.
...
- [Données utilisateur](/docs/user-data) et [Importer des données utilisateur](/docs/user-data-import) - des valeurs par utilisateur qui remplacent l'annuaire, une par une ou depuis un fichier CSV.
- [Aperçu par client de messagerie](/docs/mail-client-preview) - comment l'aperçu simule Gmail, Outlook et Apple Mail en clair et en sombre.
- [Quand Gmail tronque votre signature](/docs/gmail-sanitization) - ce que signifie l'avertissement "tronquée par Gmail" et comment y remédier.
