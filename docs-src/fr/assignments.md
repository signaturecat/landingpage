...
2. **Les affectations OU** remplacent l'affectation pour tous pour les utilisateurs de l'OU. Si un utilisateur est couvert par deux OU affectées, l'affectation **la plus récemment mise à jour** l'emporte.
...
Chaque affectation dispose d'un paramètre **Alias** qui contrôle quelles adresses reçoivent la signature. L'écriture sur les alias nécessite le scope optionnel `gmail.settings.sharing` - voir [Domain-Wide Delegation](/docs/domain-wide-delegation#quest-ce-que-jautorise-exactement).
