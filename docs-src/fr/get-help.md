...
2. **Une tâche a-t-elle échoué ?** Ouvrez les [Journaux des tâches](https://app.signature.cat/logs) et regardez les codes d'erreur par utilisateur - [Vérifier une tâche d'affectation](/docs/verify-assignments#ce-que-les-lignes-par-utilisateur-vous-disent) explique chacun d'eux et son correctif.
3. **La Domain-Wide Delegation est-elle en bonne santé ?** Si les synchronisations sont en pause, les administrateurs ont une notification "Accès Domain-Wide Delegation perdu" - relancez l'assistant depuis [Paramètres](https://app.signature.cat/settings). Voir [Domain-Wide Delegation](/docs/domain-wide-delegation#que-se-passe-t-il-si-la-dwd-est-retire-ou-un-scope-rvoqu).
...
5. **Appliquée, mais coupée dans Gmail ?** Si la tâche a réussi et que la signature semble malgré tout incomplète dans la boîte, cherchez le badge ambre **tronquée par Gmail** sur la ligne de résultat - Gmail a réécrit la signature au moment de l'enregistrer. Voir [Quand Gmail tronque votre signature](/docs/gmail-sanitization).
...
- le **lien de la tâche** (`app.signature.cat/jobs/...`) ou une capture d'écran de la ligne des [Journaux des tâches](https://app.signature.cat/logs), si une tâche est concernée,
