...
├── pricing.html            # /pricing source (EN) - see PAGES in build.mjs
├── banners.html            # /banners-generator source (EN) - free banner tool
├── pl/index.html           # GENERATED by build.mjs (do not edit)
├── de/index.html           # GENERATED
├── fr/index.html           # GENERATED
├── build.mjs               # zero-dep generator: PAGES sources + i18n.js -> per-locale pages + sitemap
...
│   ├── js/banner-generator.js # /banners-generator tool (canvas preview, PNG/HTML export, email gate)
...
## Banner generator (/banners-generator)

Free, fully client-side banner tool for email signatures (SEO page, x4
locales like every landing page). Logic lives in
`assets/js/banner-generator.js` (loaded only on that page); styles are the
`bg-*` blocks in `style.css`; copy is the `bg.*` namespace in `i18n.js`.

- **Preview & export:** one canvas render path drives both the live preview
  (devicePixelRatio-scaled) and the PNG export (exact WxH). Color/gradient
  banners can also be copied as email-ready table HTML (`text/html` +
  `text/plain` clipboard flavors). Uploaded photos stay in the browser
  (FileReader data: URL - already allowed by the Worker CSP; blob: is NOT).
